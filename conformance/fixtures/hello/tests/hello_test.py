...
# workspace is either passed as arg, taken from an env var, or this fixture dir.
# HCP_FIXTURE_WORKSPACE is the host-neutral var; the OPENCODE_* names are kept
# for back-compat with opencode-evolve's older invocation.
if len(sys.argv) > 1:
    workspace = sys.argv[1]
else:
    workspace = (os.environ.get("HCP_FIXTURE_WORKSPACE")
                 or os.environ.get("OPENCODE_EVOLVE_WORKSPACE")
                 or os.environ.get("OPENCODE_SIDECAR_WORKSPACE", ""))
    if not workspace:
